...
async def passport_verification(
    request: Request,
    passport_img: UploadFile = File(..., description="Upload an image of passport for verification."),
    ):
    try:
    ...
    return templates.TemplateResponse("verification_result.html", {"request": request, "output": output})
...
async def passport_info_extraction(
    request: Request,
    passport_img: UploadFile = File(..., description="Upload an image of passport for verification."),
    capture_img: UploadFile = File(..., description="Upload your image to verify your passport.")
    ):
    try:
        passport_base64 = await get_base64_urls(passport_img)
        ...
        traceback.print_exc()
        raise HTTPException(500,str(e))
    return templates.TemplateResponse("extraction_result.html", {"request": request, "output": output})
